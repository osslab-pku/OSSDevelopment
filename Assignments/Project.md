...
请在presentation开始前，将ppt以邮件形式发送给助教（xuww@stu.pku.edu.cn），文件命名统一为`学号-姓名-开题/中期/期末Pre.pdf/pptx`。
...
请在截止日期之前（具体时间参见今年的[课程大纲](../Syllabus.md)），将期末报告的PDF以邮件形式发送给助教（xuww@stu.pku.edu.cn），文件命名统一为`学号-姓名-期末报告.pdf`。
